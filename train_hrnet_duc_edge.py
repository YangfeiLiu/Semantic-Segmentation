...
from models.hrnetv2 import HRNet_DUC
...
global_name = 'hrnetv2_duc_edge_new'
...
class Trainer():
    def __init__(self, args):
        ...
        if args.modelname == 'hrnetv2':
            self.model = HRNet_DUC(in_channels=args.in_channels, out_channels=720, num_classes=args.num_classes)
    ...
    def __call__(self):
        ...
        if self.pretrain is not None:
            ...
            try:
                ...
                self.load_checkpoint_with_changed(use_optimizer=False, use_epoch=True, use_miou=False)
    ...
    def train_epoch(self, epoch, lr):
        ...
        for i, (image, mask, edge) in enumerate(tbar):
            ...
            if self.args.modelname == 'ocrnet':
                ...
                out, edge_ = self.model(image)
                loss = self.criterion(out, mask) + self.edge_loss(edge_.squeeze(), edge)
            loss.backward()
            self.optimizer.step()
            with torch.no_grad():
