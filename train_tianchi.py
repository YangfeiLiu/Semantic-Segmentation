...
global_name = 'resnet101_dblock_tianchi'
...
ClassMap = {"gengdi": 0, "lindi": 1, "caodi": 2, "road": 3, "chengzhen": 4, "nongcun": 5, "gongye": 6, "build": 7, "water": 8, "luodi": 9}
...
class Trainer():
    def __init__(self, args):
        ...
        self.valid_loader = DataLoader(valid_set, batch_size=args.batch_size, shuffle=True,
                                       num_workers=args.num_workers, pin_memory=True, drop_last=False)
        trainval_set = MyData(root=args.root, phase='trainval', img_mode=args.img_mode, n_classes=args.num_classes, size=args.size, scale=args.scales)
        self.trainval_loader = DataLoader(trainval_set, batch_size=args.batch_size, shuffle=True,
                                       num_workers=args.num_workers, pin_memory=True, drop_last=False)
        ...
        for epoch in range(self.start_epoch, self.epoch):
            ...
            if valid_miou > self.best_miou:
                ...
                if cnt == 20:
                    ...
                    break
        '''
        for epoch in range(self.epoch, self.epoch + 10):
            logger.info("train and valid")
            train_valid_miou = self.train_valid_epoch(epoch)
            self.lr_scheduler.LambdaLR_().step(epoch=epoch)
            self.save_checkpoint(epoch, train_valid_miou, 'final_' + global_name)
        '''
        writer.close()
    ...
    def load_checkpoint_with_changed(self, use_optimizer, use_epoch, use_miou):
        ...
        pretrain_dict = {k: v for k, v in pretrain_dict.items() if k in model_dict and 'enhance' not in k}
