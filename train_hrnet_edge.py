...
class Trainer():
    def __init__(self, args):
        ...
        train_set = HRDataEdge(root=args.root, phase='train', img_mode=args.img_mode, n_classes=args.num_classes, size=args.size, scale=args.scales)
        self.train_loader = DataLoader(train_set, batch_size=args.batch_size, shuffle=True,
                                       num_workers=args.num_workers, pin_memory=True, drop_last=False)
        valid_set = HRDataEdge(root=args.root, phase='valid', img_mode=args.img_mode, n_classes=args.num_classes, size=args.size, scale=args.scales)
        ...
        if args.modelname == 'hrnetv2':
            self.model = HRnetv2(in_channels=args.in_channels, num_classes=args.num_classes, use_ocr_head=False, usr_origin_hrnet=True)
    ...
    def train_epoch(self, epoch, lr):
        ...
        for i, (image, mask, edge) in enumerate(tbar):
            ...
            if self.args.modelname == 'ocrnet':
                ...
                _, out, pre_edge = self.model(image)
                pre_edge = F.sigmoid(pre_edge)
                loss1 = self.criterion(out.squeeze(), mask)
                loss2 = self.edge_loss(pre_edge.squeeze(), edge)
                loss = loss1 + loss2
            loss.backward()
            self.optimizer.step()
            with torch.no_grad():
    ...
    def load_checkpoint_with_changed(self, use_optimizer, use_epoch, use_miou):
        ...
        pretrain_dict = {k: v for k, v in pretrain_dict.items() if k in model_dict and 'module.last_layer.3' not in k}
